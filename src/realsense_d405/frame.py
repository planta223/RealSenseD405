"""Immutable D405 sensor frames and a latest-frame buffer."""
...
class D405Frame:
    """One depth-to-color-aligned D405 sensor frame.

    ``depth_raw`` preserves Z16 sensor units for lossless recording.
    ``depth_m`` is the same aligned depth converted to float32 meters.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if (
            not isinstance(self.depth_raw, np.ndarray)
            or self.depth_raw.dtype != np.uint16
        ):
            raise TypeError("depth_raw must be a uint16 NumPy array.")
        if self.depth_raw.ndim != 2:
            raise ValueError("depth_raw shape must be (H, W).")
        ...
        if self.depth_m.ndim != 2:
            raise ValueError("depth_m shape must be (H, W).")
        if (
            self.rgb.shape[:2] != self.depth_raw.shape
            or self.depth_raw.shape != self.depth_m.shape
        ):
